    ...
    : TGMainFrame(p, w, h)
{
  ...
  dNRunsSeq = 1;   // Originally   dNRunsSeq = 1
  dNRunSeqCnt = 0;
...
void CMMonitor::StartDataCollection()
{
  ...
  while (1)
  {
    ...
    if (RUN_START)
    {
      ...
      // flag = 1;
      if (dNRunsSeq == 1){  // originally this is where the data plotting of the GUI was happened
        cout << "[Wait]: Filling root tree " << endl;
        // FillDataPlots();
        FillRootTree();
        cout << "[Done] " << endl;
        cout << "[Exit] " << endl;
      ...
      {
        cout << "Filling root tree original " << endl;
        FillRootTree();
...
Int_t CMMonitor::OpenDataFile(ERFileStatus status, const char *file)
{
  ...
  if (strcmp(".dat", strrchr(filename, '.')))
    return PROCESS_FAILED;
